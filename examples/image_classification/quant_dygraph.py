...
def main():
    if FLAGS.enable_quant:
        dygraph_qat = DygraphQuantAware(weight_quantize_type='abs_max',
    ...
    if FLAGS.enable_quant:
        quant_output_dir = os.path.join(FLAGS.output_dir, FLAGS.arch + "_quant",
                              time.strftime('%Y-%m-%d-%H-%M',
                                            time.localtime()))
        dygraph_qat.save_infer_quant_model(dirname=quant_output_dir,
                model=model, input_shape=(3,224,224), input_dtype='float32',
                feed=[0], fetch=[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--train_batchs", default=-1, type=int, help="")
    parser.add_argument(
        "--test_batchs", default=-1, type=int, help="")
    parser.add_argument(
