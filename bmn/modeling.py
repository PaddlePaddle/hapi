...
class BMN(paddle.nn.Layer):
    ...
    def __init__(self, tscale, dscale, prop_boundary_ratio, num_sample,
        ...
        self.sample_mask = paddle.static.create_parameter(
...
class BmnLoss(paddle.nn.Layer):
    ...
    def _get_mask(self):
        ...
        for idx in range(self.dscale):
            ...
            bm_mask_list.append(mask_vector)
        bm_mask_array = np.array(bm_mask_list, dtype=np.float32)
        self_bm_mask = paddle.static.create_global_var(
            shape=[self.dscale, self.tscale],
            value=0,
            dtype=DATATYPE,
            persistable=True)
        paddle.assign(bm_mask_array, self_bm_mask)
