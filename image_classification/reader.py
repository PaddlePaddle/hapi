...
class ImageNetReader:
    ...
    def _reader_creator(self,
        ...
        def reader():
            def read_file_list():
                ...
                if num_trainers > 1 and mode != 'train':
                    total_size = len(full_lines)
                    padded_total_size = ((len(full_lines) - 1) // settings.batch_size + 1) * settings.batch_size
                    full_lines += full_lines[:padded_total_size - total_size]
                #     print('total size:', len(full_lines), total_size, padded_total_size)
                # else:
                #     print('total size:', len(full_lines))
                batch_data = []
                ...
                if len(batch_data) > 0 and mode != 'train':
                    yield batch_data

            return read_file_list

        data_reader = reader()
        if (mode == 'train' or mode == 'val') and num_trainers > 1:
            ...
            if mode == 'train':
                ...
                data_reader = distributed_batch_reader_val(
                    data_reader)
        ...
        ret = fluid.io.xmap_readers(
            mapper,
            data_reader,
            4, #settings.reader_thread,
            4, #settings.reader_buf_size,
            order=False)
    ...
    def train(self, settings):
        ...
        Returns:
            train reader
        """
        settings.data_dir = settings.data
        ...
        settings.crop_size = 224
        settings.interpolation = None
        settings.resize_short_size = 256
        ...
        reader = self._reader_creator(
            ...
            color_jitter=False,
            rotate=False,
            data_dir=os.path.join(settings.data_dir, 'val_paddle'))
...
def distributed_batch_reader_val(batch_reader):
    """
